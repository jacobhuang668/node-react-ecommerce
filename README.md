...
│
├── backend/ # 后端代码目录
│ ├── config/ # 配置文件（例如数据库、JWT 等）
│ ├── models/ # 数据模型（例如用户、产品、订单等）
│ ├── routes/ # API 路由
│ ├── controllers/ # 路由控制器逻辑
│ ├── data/ # 示例数据（可能包含用户和产品数据）
│ ├── utils/ # 实用工具函数（如错误处理、中间件）
│ ├── server.js # 后端入口文件
│ └── .env # 环境变量文件
│
├── frontend/ # 前端代码目录
│ ├── public/ # 静态资源文件（HTML、图标等）
│ ├── src/ # 前端源码
│ │ ├── components/ # 可复用组件（例如导航栏、按钮等）
│ │ ├── screens/ # 页面级组件（例如产品详情页、购物车页）
│ │ ├── actions/ # Redux 动作（如果使用 Redux）
